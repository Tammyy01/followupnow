  ...
  return (
    <div className="w-full h-[132px] rounded-[14px] bg-[#FAF8F5]/60 border border-[#F0EDE8] overflow-hidden relative">
      <svg viewBox="0 0 320 120" className="w-full h-full">
        ...
        />
        <circle cx="118" cy="36" r="4" fill="#6B8F7E" stroke="white" strokeWidth="2" />
        <circle cx="266" cy="34" r="4" fill="#6B8F7E" stroke="white" strokeWidth="2" />
      </svg>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-[#EDE9E1] text-[#1E2A26] antialiased selection:bg-[#6B8F7E]/20 overflow-x-hidden">
      <div className="max-w-[1440px] mx-auto px-3 lg:px-6 py-4 lg:py-6 flex gap-5 min-w-0">
        ...
        <main className="flex-1 min-w-0">
          {/* Header */}
          <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6 min-w-0">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-full bg-[#6B8F7E] flex items-center justify-center shrink-0">
                <SunIcon />
              </div>
              <div>
            ...
            <div className="flex items-center gap-3 w-full lg:w-auto min-w-0">
              <div className="flex-1 lg:w-[320px] h-[44px] bg-white rounded-full flex items-center px-2 shadow-sm border border-white/70 min-w-0">
                <input
                  value={search}
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-full bg-[#11181C] text-white flex items-center justify-center shrink-0"
                >
              ...
              <button
                ...
                className="w-11 h-11 rounded-full bg-white shadow-sm border border-white/70 flex items-center justify-center relative shrink-0"
              >
                <Message2 variant="Linear" size={18} color="#8EA09A" />
                <span className="absolute top-[9px] right-[10px] w-2 h-2 bg-[#E34D4D] rounded-full ring-2 ring-white" />
              </button>
              <button
                onClick={() => showToast('Notifications')}
                className="w-11 h-11 rounded-full bg-white shadow-sm border border-white/70 flex items-center justify-center shrink-0"
              >
          ...
          <div className="grid grid-cols-12 gap-4 lg:gap-5 min-w-0">
            {/* Spent this month */}
            <div className="col-span-12 md:col-span-6 lg:col-span-3 bg-white rounded-[24px] p-5 shadow-sm border border-white/60 flex flex-col min-h-[148px]">
              <div className="flex items-start justify-between">
                <div className="flex flex-col">
                  <span className="text-[12px] font-medium text-[#9AA9A1] uppercase tracking-wide">
                    Total Users
                  </span>
                  <span className="text-[30px] font-bold tracking-tight mt-1">{data.totalUsers.toLocaleString()}</span>
                </div>
                <button
                  onClick={() => showToast('Spent details')}
                  className="w-8 h-8 rounded-full border border-[#E3EAE6] flex items-center justify-center text-[#7D9B8B] hover:bg-[#F5F7F6]"
                >
                  <ArrowUp3 variant="Linear" size={14} color="currentColor" />
                </button>
              </div>
              <div className="mt-auto pt-6 flex items-end gap-[5px] h-[52px]">
                {[44, 28, 36, 22, 14].map((h, i) => (
                  <div
                    key={i}
                    className="flex-1 rounded-[6px]"
                    style={{ height: `${h}px`, background: i === 0 ? '#6B8F7E' : '#D9E3DD' }}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="col-span-12 md:col-span-6 lg:col-span-3 bg-white rounded-[24px] p-5 shadow-sm border border-white/60 flex flex-col min-h-[148px]">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="text-[30px] font-bold tracking-tight mt-1">{data.signupsLast30d.toLocaleString()}</div>
                </div>
            ...
            <div className="col-span-12 md:col-span-6 lg:col-span-3 bg-white rounded-[24px] p-5 shadow-sm border border-white/60 flex flex-col min-h-[148px]">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="text-[30px] font-bold tracking-tight mt-1">{(data.lifetimeCount + data.trialActiveCount).toLocaleString()}</div>
                </div>
            ...
            {/* Activity */}
            <div className="col-span-12 md:col-span-6 lg:col-span-3 rounded-[24px] p-5 shadow-sm border border-white/10 flex flex-col min-h-[148px] text-white bg-gradient-to-br from-[#8AA99B] to-[#6D8F7E] relative overflow-hidden">
              <div className="flex items-start justify-between relative z-10">
                <div>
                  <span className="text-[12px] font-medium text-white/70 uppercase tracking-wide">
                    Conversion Rate
                  </span>
                  <div className="text-[30px] font-bold tracking-tight mt-1">{(((data.lifetimeCount + data.trialActiveCount) / (data.totalUsers || 1)) * 100).toFixed(1)}%</div>
                </div>
                <button
                  onClick={() => showToast('Activity')}
                  className="w-8 h-8 rounded-full bg-white/20 backdrop-blur flex items-center justify-center hover:bg-white/30"
                >
            ...
            {/* Balance */}
            <div className="col-span-12 lg:col-span-6 bg-white rounded-[24px] p-6 shadow-sm border border-white/60 min-w-0">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[13px] font-medium px-3 py-1.5 rounded-full bg-[#F2F2F0] border border-[#E9E6E0] flex items-center gap-1.5"
                >
                  {period} <ArrowDown2 variant="Linear" size={12} color="currentColor" />
                </button>
              </div>
              <div className="mt-5 grid grid-cols-2 gap-4">
                <div className="bg-[#F7F6F3] rounded-[16px] p-4 border border-[#EFECE6]">
                  <div className="text-[11px] font-semibold tracking-widest text-[#9AA9A1] uppercase">
                    Trials
                  </div>
                  <div className="flex items-baseline gap-2 mt-1.5">
                    <span className="text-[22px] font-bold">{data.trialActiveCount.toLocaleString()}</span>
                ...
                <div className="bg-[#F7F6F3] rounded-[16px] p-4 border border-[#EFECE6]">
                  ...
                  <div className="flex items-baseline gap-2 mt-1.5">
                    <span className="text-[22px] font-bold">{data.signupsLast7d.toLocaleString()}</span>
            ...
            {/* Earnings gauge */}
            <div className="col-span-12 md:col-span-6 lg:col-span-3 bg-white rounded-[24px] p-6 shadow-sm border border-white/60 flex flex-col">
              ...
              <div className="text-[30px] font-bold tracking-tight mt-2 text-[#5A7D6E]">
                {data.lifetimeCount.toLocaleString()}
              </div>
              ...
              <div className="mt-6 flex-1 flex flex-col items-center justify-center">
                ...
                <div className="mt-2 text-[12px] text-[#9AA9A1] font-medium">Achieved</div>
              </div>
            </div>

            {/* Profile */}
            <div className="col-span-12 md:col-span-6 lg:col-span-3 bg-white rounded-[24px] p-6 shadow-sm border border-white/60 flex flex-col items-center text-center">
              <div className="w-[86px] h-[86px] rounded-full bg-[#F5EEE0] flex items-center justify-center text-[40px] border border-[#EDE6D8] shadow-inner">
              ...
              <div className="mt-8 w-full grid grid-cols-3 divide-x divide-[#EEEAE3] border-t border-[#EEEAE3] pt-5">
                <div>
                  <div className="text-[18px] font-bold leading-none">{data.lifetimeCount}</div>
                  <div className="text-[10.5px] font-semibold tracking-widest uppercase text-[#A9B7B0] mt-1.5">
                    Lifetime
                  </div>
                </div>
                <div>
                  <div className="text-[18px] font-bold leading-none">{data.trialActiveCount}</div>
                  <div className="text-[10.5px] font-semibold tracking-widest uppercase text-[#A9B7B0] mt-1.5">
                    Trial
                  </div>
                </div>
                <div>
                  <div className="text-[18px] font-bold leading-none">{data.noPlanCount}</div>
                  <div className="text-[10.5px] font-semibold tracking-widest uppercase text-[#A9B7B0] mt-1.5">
                    No Plan
                  </div>
                </div>
            ...
            <div className="col-span-12 lg:col-span-6 bg-white rounded-[24px] p-6 shadow-sm border border-white/60 overflow-hidden min-w-0">
              <div className="flex flex-col lg:flex-row gap-6 h-full">
                <div className="flex-1 min-w-0">
                  ...
                  <button
                    ...
                    className="mt-6 h-11 px-5 rounded-[12px] bg-[#6D8F7E] hover:bg-[#5E7F6F] text-white text-[13.5px] font-semibold shadow-sm transition"
                  >
                    Add New Card +
                  </button>
                </div>
                <div className="relative flex-1 min-h-[200px] lg:min-h-0 flex items-center justify-center overflow-hidden">
                  <div className="relative w-[260px] h-[180px] [perspective:900px] shrink-0 scale-[0.9] lg:scale-100 origin-center">
                    {/* Back card */}
                    <div
                      className="absolute left-8 top-[78px] w-[200px] h-[120px] rounded-[16px] bg-[#1F2A2E] shadow-xl border border-white/10 p-4 flex flex-col justify-between"
                    ...
                    {/* Middle card */}
                    <div
                      className="absolute left-4 top-[38px] w-[200px] h-[120px] rounded-[16px] bg-[#8AA99B] shadow-xl border border-white/20 p-4 flex flex-col justify-between"
                    ...
                    {/* Front card */}
                    <div className="absolute left-0 top-0 w-[220px] h-[132px] rounded-[18px] bg-gradient-to-br from-[#EEF3EF] to-[#D8E4DD] shadow-2xl border border-white p-4 flex flex-col justify-between">
            ...
            {/* Your Transfers */}
            <div className="col-span-12 md:col-span-6 lg:col-span-3 bg-white rounded-[24px] p-6 shadow-sm border border-white/60">
              ...
              <div className="mt-5 space-y-5">
                {data.recentUsers.map((u, i) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    />
                    <div
                      className="w-9 h-9 rounded-full overflow-hidden shrink-0 flex items-center justify-center text-[11px] font-bold"
            ...
            {/* Keep you safe! */}
            <div className="col-span-12 md:col-span-6 lg:col-span-3 bg-white rounded-[24px] p-6 shadow-sm border border-white/60 flex flex-col items-center text-center">
              <div className="w-24 h-24 rounded-full bg-[#EEF4EF] flex items-center justify-center mt-1">
              ...
              <button
                ...
                className="mt-6 w-full h-11 rounded-[12px] bg-[#6D8F7E] hover:bg-[#5E7F6F] text-white text-[13.5px] font-semibold transition shadow-sm"
              >
